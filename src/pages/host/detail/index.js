...
function HostDetail(props) {
    ...
    const [hostInfo, setHostInfo] = useState(InitState.hostInfo);
    const [hostBaseInfo, setHostBaseInfo] = useState(InitState.hostBaseInfo);
    ...
    function getBaseInfo({ refresh = false }) {
        ...
        XHR.baseInfo(id, params).then(res => {
          // json字符串数据不规范
            const detail = res.detail ? JSON.parse(res.detail.replace(/'([^']*)'/g, '"$1"')
                                                ...
                                                .replace(/"None"/g, 'None')
                                                .replace(/None/g, '"none"')) : {};
            const { ansible_uptime_seconds = '', ansible_all_ipv4_addresses = '', ansible_processor_count = '', ansible_processor = '', ansible_mounts = [], ansible_devices = [] } = detail;
            setHostBaseInfo({
              ...hostBaseInfo,
     ...
     * 组装磁盘挂载
     */
    function transformMounts(mounts) {
    ...
    function getInfo({ refresh = false }) {
      ...
      if (refresh) setLoading(true);
        XHR.info(id, params).then(res => {
          const { ip, admin_user, port} = res;
          const hostConf = res.host_configuration ? JSON.parse(res.host_configuration.replace(/'([^']*)'/g, '"$1"')) : {};
