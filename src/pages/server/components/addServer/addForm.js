...
class AddServerForm extends React.Component {
    ...
    getHostIpList = () => {
        ...
        this.state.API.host.list(param).then((res) => {
            this.setState({
    ...
    remove = k => {
      ...
      refreshHostIpList = () => {
        const { form } = this.props;
        // 由于onChange无法立即获取schemas的值 利用settime解决
        setTimeout(() => {
...
export default Form.create()(AddServerForm);
