...
import { API } from '@/api';
const { Option } = Select;
class InstallServerForm extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            frameworkList: {
    ...
    onDbTypeChange = (val) => {
        ...
        this.props.changeForm({dbtype: val, framework: ''});
    }
      ...
      refreshHostIpList = () => {
        ...
        setTimeout(() => {
            ...
            const existIpId = hostList.map(item => item.hostname); // 已经选择的主机ip的id集合
    ...
    render () {
        ...
        return (
            <Form className="add-host-form" {...formItemLayout}>
                ...
                {formData.framework && keys.map((k,index) => (
...
export default Form.create()(InstallServerForm);
