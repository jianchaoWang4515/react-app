...
function Report(props) {
    ...
    function getData(id, { page = 1, page_size = 20}) {
        const params = {
            serviceid: id,
            page,
            page_size
        ...
        XHR.report.send({params}).then(res => {
    ...
    function goTo(text) {
        ...
        newWindow.location.href = `http://10.186.177.21:8001/${text}`;
    }
    return (
        <div className="app-page">
            <PageTitle title={`巡检报告-Oracle-${props.location.state ? props.location.state.servicename : ''}`}></PageTitle>
            <Table
                className="m-t-24"
                size="middle"
                loading={loading}
