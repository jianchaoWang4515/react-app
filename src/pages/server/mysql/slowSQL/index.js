...
import { Table, Button, message, DatePicker, Pagination, Typography, Tooltip, Drawer } from 'antd';
...
function SlowSQL(props) {
  ...
  function getData({serverid, time = '', page = 1, pageSize = 20}) {
    const params = {
        page,
        pageSize,
        serviceid: serverid
    ...
    XHR.slowSQLList.send({params}).then(res => {
  ...
  function onOk(dates) {
    const time = [dates[0].format('YYYY-MM-DD HH:MM:ss'), dates[1].format('YYYY-MM-DD HH:MM:ss')]
    setSearchState({
      ...searchState,
      time
    });
      ...
      <div className="app-page">
        ...
        >
          ...
          <Column
            ...
            render={(text, record) => (
              <>
                <Button size="small" type="primary" onClick={(e) => {setVisible(true);setActivityData(record);setActionType('sample')}}>样例</Button>
                <Button className="m-l-16" size="small" type="primary" onClick={(e) => {setVisible(true);setActivityData(record);setActionType('optimize')}}>优化</Button>
              </>
            )}
          />
        </Table>
